...
def test_grid_tiles(klass, dims: int) -> None:
    rasters = []
    for x in range(dims):
        for y in range(dims):
            raster = RasterLayer(
                gdal_dataset_with_data(
                    (10.0 * x, -10 * y), 2.0, np.full((5, 5), (y * dims) + x)
                )
            )
...
def test_overlapping_tiles(klass, dims: int) -> None:
    rasters = []
    for x in range(dims):
        for y in range(dims):
            raster = RasterLayer(
    ...
    assert (
        data
        == generate_child_tile(
            0, 0, (dims * 5) + 2, (dims * 5) + 2, (dims * 5) + 2, (dims * 5) + 2
        )
    ).all()
...
def test_multipe_tiles_with_window(klass, dims: int) -> None:
    rasters = []
    for x in range(dims):
        for y in range(dims):
            raster = RasterLayer(
...
def test_overlapping_tiles_with_window(klass, dims: int) -> None:
    rasters = []
    for x in range(dims):
        for y in range(dims):
            val = (y * dims) + x
    ...
    assert (
        data
        == generate_child_tile(
            2, 2, (5 * dims) - 4, (5 * dims) - 4, (dims * 5) + 2, (dims * 5) + 2
        )
    ).all()
...
def test_multipe_tiles_with_missing_tile(klass, dims: int, remove: int) -> None:
    rasters = []
    for x in range(dims):
        for y in range(dims):
            val = (y * dims) + x
            if val == remove:
                continue
            raster = RasterLayer(
                gdal_dataset_with_data((10.0 * x, -10 * y), 2.0, np.full((5, 5), val))
            )
            rasters.append(raster)
...
def test_oversized_tiles_with_missing_tile(klass, dims: int, remove: int) -> None:
    rasters = []
    for x in range(dims):
        for y in range(dims):
            val = (y * dims) + x
            ...
            raster = RasterLayer(
                gdal_dataset_with_data(
                    (-2 + (10 * x), 2 + (-10 * y)), 2.0, np.full((7, 7), val)
                )
            )
            rasters.append(raster)
...
def test_read_zero_pixels(klass, size: tuple[int,int]) -> None:
    dims = 2
    rasters = []
    for x in range(dims):
        for y in range(dims):
...
def test_oversized_tiles_with_missing_tile_row_slices(klass, dims, remove) -> None:
    rasters = []
    for x in range(dims):
        for y in range(dims):
            val = (y * dims) + x
...
def test_multipe_tiles_with_missing_tile_row_slices(klass, dims, remove) -> None:
    rasters = []
    for x in range(dims):
        for y in range(dims):
            val = (y * dims) + x
...
@pytest.mark.parametrize("klass", [GroupLayer, TiledGroupLayer])
def test_overlapping_tile_with_nodata(klass) -> None:
    area1 = Area(-10, 10, 10, -10, yg.MapProjection("epsg:4326", 2.0, -2.0))
