...
def test_save_multiband_non_overlapping_area() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        expected_window = Window(0, 0, 44, 44)
        ...
        for idx in range(5):
            with yg.read_raster(filename, band=idx + 1) as layer:
                ...
                expected = np.pad(data[idx], region)
                result = layer.read_array(0, 0, 44, 44)
...
def test_save_multiband_calc() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        filename = Path(tmpdir) / "test.tif"
        projection = yg.MapProjection("esri:54009", 100.0, -100.0)
        ...
        data = [np.full((4, 4), idx + 1) for idx in range(5)]
        layers = [yg.from_array(datum, (0, 0), projection) for datum in data]
        calcs = [layer * 2 for layer in layers]

        yg.to_geotiff(filename, calcs)

        for idx in range(5):
            with yg.read_raster(filename, band=idx + 1) as layer:
                assert layer.area == layers[idx].area
                result = layer.read_array(0, 0, 4, 4)
                assert (result == data[idx] * 2).all()

def test_save_multiband_calc_non_overlapping_area() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        filename = Path(tmpdir) / "test.tif"
        projection = yg.MapProjection("esri:54009", 1.0, -1.0)
        ...
        for idx in range(5):
            with yg.read_raster(filename, band=idx + 1) as layer:
                ...
                expected = np.pad(data[idx] * 2, region)
                result = layer.read_array(0, 0, 44, 44)
                assert (expected == result).all()
